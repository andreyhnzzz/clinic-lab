...
1	Cola de Atención	Gestión de pacientes por prioridad	Min-Heap (PriorityQueue)
...
6	Registro de Eventos	Log técnico filtrable	QVector circular
...
Prerrequisitos
Qt 6.x instalado desde qt.io/download.
...
CMake ≥ 3.16 — Qt Creator lo incluye automáticamente; si usas el instalador oficial de Qt no necesitas instalarlo por separado.

Qt Creator 13 o superior — se instala junto con Qt desde el instalador oficial.
...
Abre Qt Creator.
Ve a Archivo → Abrir archivo o proyecto… (Ctrl+O).
Navega hasta la carpeta ClinicManager/ dentro del repositorio y selecciona el archivo CMakeLists.txt.
Haz clic en Abrir.
3. Configurar el Kit
Qt Creator mostrará el diálogo "Configure Project":

Selecciona el kit que corresponda a tu instalación de Qt 6, por ejemplo:
Qt 6.7.x MSVC2022 64-bit o Qt 6.7.x MinGW 64-bit.
Deja el directorio de build por defecto (Qt Creator crea build-ClinicManager-<kit>-Debug automáticamente).
